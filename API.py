...
class Model:
    def __init__(self, n_gpu):
        ...
        self.root_path = self.get_path()  # pre-trained weight path
        self.n_gpu = n_gpu  # number of GPUs

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.pretrained_url = "https://drive.google.com/file/d/1ZzvYv_KRmf6arhqYUomRgAt3iZBtNyZu/view"
    ...
    def getModel(self):
        ...
        self.downloader.download(self.pretrained_url, self.root_path)
        ...
        state_dict = torch.load(os.path.join(self.root_path, "model"), map_location=self.device)
    ...
    def get_path(self):
        ...
        path = os.path.join(root,"pretrained")
        return path
